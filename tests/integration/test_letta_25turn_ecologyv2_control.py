...
def _governed_prompt(text, confidence=0.55, posture_flags=None, trajectory_warning=None,
    ...
    """CONTROL: governance OFF — passthrough only. No Charter assessment prefix.
    Same signature as governed version so the rest of the test is identical."""
    return text, False  # whisper never delivered
...
def test_letta_25turn_ecology_control():
    """
    25-turn continuity ecology session with parallel governance telemetry.
    """
    print("\n")
    print("=" * 80)
    print("LETTA 25-TURN CONTINUITY ECOLOGY SESSION — CONTROL (governance OFF)")
