...
def _extract(result):
    ...
    for msg in result.messages:
        ...
        if "tool_call" in mt.lower():
            ...
            if tc:
                tool_calls.append(f"{tc.name}({tc.arguments[:50]}...)")
...
def test_letta_25turn_ecology():
    ...
    try:
        for turn_num, phase, prompt_text in TURNS:
            ...
            # Note (Opus): this is "cold within session" not "cold across sessions" —
            # Letta still has conversation history in context. A true cross-session
            # cold restart would require a new agent or cleared recall storage.
            if phase == "cold_restart":
                ...
                governed, delivered = _governed_prompt(
                    prompt_text,
                    confidence,
                ...
                print(f"\n  [T{turn_num:02d} | {phase}] whisper={'Y' if delivered else 'n'}")
